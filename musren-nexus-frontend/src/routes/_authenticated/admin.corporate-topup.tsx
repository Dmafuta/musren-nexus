...
import {
  ArrowLeft, CheckCircle2, Search, ShieldAlert, Sparkles,
  XCircle, PhoneCall, Mail, MessageCircle, Wallet,
} from "lucide-react";
...
type Inquiry = {
  id: string; company: string; industry: string | null; contact_name: string;
  email: string; phone: string; role: string | null; network: string;
  estimated_volume: string; frequency: string; use_cases: string[];
  preferred_contact: "Email" | "Phone call" | "WhatsApp"; notes: string | null;
  status: Status; status_notes: string | null; contacted_at: string | null;
...
const CONTACT_ICON = {
  Email: Mail, "Phone call": PhoneCall, WhatsApp: MessageCircle,
} as const;
...
function AdminCorporateTopup() {
  ...
  const { data: inquiries = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<{ data: Inquiry[] }>("/api/admin/corporate-topup");
      return res.data ?? [];
    },
  });
  ...
  const counts = useMemo(() => {
    const base: Record<Status | "all", number> = { all: inquiries.length, new: 0, contacted: 0, qualified: 0, rejected: 0 };
    inquiries.forEach((i) => { base[i.status] += 1; });
    return base;
  }, [inquiries]);

  const updateStatus = useMutation({
    mutationFn: async ({ id, next }: { id: string; next: Status }) => {
      await api.patch(`/api/admin/corporate-topup/${id}`, { status: next });
    },
    onSuccess: (_d, vars) => {
  ...
  if (loading) return <SiteLayout><Section title="Loading admin…" /></SiteLayout>;

  if (!isAuthorized) {
    return (
      <SiteLayout>
        <Section eyebrow="Access denied" title="You don't have access to this dashboard"
          description="This area is restricted to Musren admins and staff.">
          <div className="glass rounded-2xl p-6 flex items-start gap-4 max-w-xl">
            ...
            <div className="text-sm text-muted-foreground">
              Once an admin grants your account a role, refresh this page.
            </div>
          </div>
          <div className="mt-6">
  ...
  return (
    <SiteLayout>
      <Section
        eyebrow="Admin"
        title={<span className="inline-flex items-center gap-3"><Wallet className="size-7 text-primary" /> Corporate Topup leads</span>}
      ...
      >
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-8">
          {(["all", "new", "contacted", "qualified", "rejected"] as const).map((k) => (
            <button key={k} onClick={() => setStatus(k)}
              className={`glass rounded-2xl p-4 text-left transition hover:bg-white/[0.06] ${status === k ? "ring-1 ring-primary/60" : ""}`}>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search company, name, email, phone…" className="pl-9 glass" />
          </div>
          <Select value={network} onValueChange={setNetwork}>
            <SelectTrigger className="w-[180px] glass"><SelectValue placeholder="Network" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="rounded-2xl glass-strong overflow-hidden border-gradient">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">Loading inquiries…</TableCell></TableRow>
              ) : filtered.length === 0 ? (
                <TableRow>
                  ...
                  return (
                    <TableRow key={row.id} className="align-top">
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">{row.contact_name}{row.role ? ` · ${row.role}` : ""}</div>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1 max-w-[260px]">
                          {row.use_cases.map((u) => <Badge key={u} variant="secondary" className="text-[10px]">{u}</Badge>)}
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] ${meta.cls}`}>{meta.label}</span>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="inline-flex flex-wrap gap-1.5 justify-end">
                          {row.status !== "contacted" && row.status !== "qualified" && (
                            <Button size="sm" variant="outline" className="glass" disabled={updateStatus.isPending}
                              onClick={() => updateStatus.mutate({ id: row.id, next: "contacted" })}>
                              <PhoneCall className="size-3.5 mr-1" /> Contacted
                            </Button>
                          )}
                          {row.status !== "qualified" && (
                            <Button size="sm" className="bg-emerald-500/90 hover:bg-emerald-500 text-emerald-950"
                              disabled={updateStatus.isPending}
                              onClick={() => updateStatus.mutate({ id: row.id, next: "qualified" })}>
